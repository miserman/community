...
page_navbar(
  ...
  list(
    ...
    items = list(
      ...
      input_select(
        "Summary Level",
        options = c("dataset", "children", "all"),
        default = "dataset",
        display = c("All Regions", "Selected Super-Region", "Showing Regions"),
        id = "settings.summary_selection",
        floating_label = FALSE,
        note = paste(
      ...
      input_switch(
        "Show Point Overlay",
        default_on = TRUE,
        id = "settings.map_overlay"
      ),
      input_switch(
        "Show Background Shapes",
        default_on = TRUE,
        id = "settings.background_shapes"
      ),
      input_switch(
        "Background Shapes On Top",
        default_on = TRUE,
        id = "settings.background_top"
      ),
      input_select(
        "Animations",
        c("fly", "zoom", "none"),
      ...
      input_number(
        ...
        "settings.background_polygon_outline",
        default = 2,
        step = .5,
        floating_label = FALSE
      ),
      ...
      input_select(
        "Overlay Circle Property",
        "overlay_properties",
        id = "settings.circle_property",
        floating_label = FALSE,
...
page_menu(
  input_checkbox(
    "Starting Layer",
    c("county", "tract", "block_group"),
    0,
    c("Counties", "Census Tracts", "Block Groups"),
    id = "shape_type",
    multi = FALSE,
    note = "Which geographies to show at the state level."
  ),
  page_section(
    type = "row form-row",
    wraps = "row",
    ...
    input_combobox(
      ...
      dataview = "primary_view",
      id = "selected_tract",
      selection_subset = "tract_subset",
      clearable = TRUE
    ),
    conditions = c(
      "lock: !selected_tract",
      "shapes == tract || shapes == block_group"
    )
  ),
  page_section(
    type = "col",
